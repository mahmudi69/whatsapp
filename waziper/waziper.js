...
const WAZIPER = {
		...
		await WA.ev.on(
			...
			async ({
				...
				if (lastDisconnect != undefined && lastDisconnect.error != undefined) {
					...
					if (
						DisconnectReason.restartRequired == statusCode ||
				...
				switch (connection) {
					...
					case "open":
						...
						if (session) {
							...
							var account = await Common.db_get("sp_accounts", [
								{ token: instance_id },
							]);
							if (!account) {
	...
	get_qrcode: async function (instance_id, res) {
		...
		if (client == undefined) {
			return res.json({
				status: "error",
				message: "The WhatsApp session could not be found in the system",
			});
		}
		...
		if (client.qrcode == undefined || client.qrcode == false) {
			return res.json({
				status: "error",
				message: "The system cannot generate a WhatsApp QR code",
			});
		}
